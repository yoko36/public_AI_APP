...
// ==================================================
// // プロジェクト一覧を取得するメソッド（GET）
// ==================================================
export async function GET(req: Request) {
  const backend = process.env.BACKEND_INTERNAL_URL!;
  const url = new URL(`${backend}/api/v1/projects`);
...
// ==================================================
// // 新規プロジェクトを追加するメソッド（POST）
// ==================================================
export async function POST(req: Request) {
  const backend = process.env.BACKEND_INTERNAL_URL!;
  const url = `${backend}/api/v1/projects`;
  ...
  try {
    up = await fetch(url, {
      method: "POST",
      headers: forwardHeaders(req),
      body: await req.text(), // JSONそのまま中継
      cache: "no-store",
      signal: (req as any).signal,
    });
  } catch (e: any) {
    console.error("[/api/v1/projects] upstream fetch error:", e?.message ?? e);
  ...
  if (!up.ok) {
    console.error("[/api/v1/projects] upstream", up.status, text);
