...
// ==================================================
// // スレッドに関する情報を変更するメソッド（UPDATE）
// ==================================================
export async function PATCH(req: Request, ctx: { params: { id: string } }) {
  const backend = process.env.BACKEND_INTERNAL_URL!;
  const url = `${backend}/api/v1/threads/${encodeURIComponent(ctx.params.id)}`;
  ...
  try {
    ...
    console.error("[/api/v1/threads/:id] upstream fetch error:", e?.message ?? e);
  ...
  if (!up.ok) {
    console.error("[/api/v1/threads/:id] upstream", up.status, text);
